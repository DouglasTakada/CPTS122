...
void FitnessAppWrapper::performUserSelection(const int &option)
{
	...
	case 2: // load weekly EP
		...
		cout << "Exercise plans loaded from dietPlans.txt!" << endl;
		break;
	...
	case 4: // store weekly EP
		cout << "Exercise plans stored to dietPlans.txt!" << endl;
		break;
...
{
	this->displayWeeklyPlan(this->mEPList); // display a weekly exercise plan to the screen!
}
void FitnessAppWrapper::storeWeeklyExercisePlan() // this will call the private storeWeeklyPlan() function
{
	// We want to start overwriting from the very beginning of the "exercisePlan.txt" file.
	...
	{
		this->mFstrDP.open("exercisePlans.txt"); // opens in input/output mode by default with fstream object
		this->storeWeeklyPlan(this->mFstrEP, this->mEPList);
		this->mFstrEP.close();
	}
}
